...
-- bit    0: Unused        =  0
  ...
  if username then flags = flags + MQTT.Utility.shift_left(1, 7) end
  if password then flags = flags + MQTT.Utility.shift_left(1, 6) end

  if (will_topic == nil) then
    flags = flags + 0x02       -- Clean session, no last will
  else
    local flags
